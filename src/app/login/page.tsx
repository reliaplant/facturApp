...
const Login = () => {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      const userCredential = await userService.loginWithGoogle();
      ...
      if (userData?.role === 'cliente') {
        // Cliente - va a su portal de contabilidad
        router.push('/mi-contabilidad');
      } else {
        // super_admin, admin, contador -> dashboard
        router.push('/dashboard');
      }
      ...
      <div className="w-full max-w-md p-8 space-y-8">
        ...
        <div className="space-y-4">
          ...
          <button
            ...
            disabled={loading}
            className={`w-full flex items-center justify-center gap-3 py-3 px-4 border border-gray-300 rounded-md bg-white hover:bg-gray-50 shadow-sm hover:shadow focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-violet-500 transition-all ${loading ? 'opacity-70 cursor-not-allowed' : ''}`}
          >
