...
export const cfdiService = {
  ...
  async saveInvoices(clientId: string, invoices: CFDI[]): Promise<any> {
    try {
  ...
  async getInvoices(clientId: string, year?: number): Promise<CFDI[]> {
    try {
      const invoicesRef = collection(db, 'clients', clientId, 'cfdi');
      ...
      querySnapshot.forEach((doc) => {
        invoices.push(doc.data() as CFDI);
      });
  ...
  // Get invoice by ID
  async getInvoiceById(clientId: string, invoiceId: string): Promise<CFDI | null> {
    try {
      const invoiceRef = doc(db, 'clients', clientId, 'cfdi', invoiceId);
      const invoiceDoc = await getDoc(invoiceRef);
      
      if (invoiceDoc.exists()) {
        return invoiceDoc.data() as CFDI;
      }
  ...
  async updateInvoice(clientId: string, invoiceId: string, updateData: Partial<CFDI>): Promise<void> {
    try {
      const invoiceRef = doc(db, 'clients', clientId, 'cfdi', invoiceId);
   ...
   */
  _calculateGravados(invoice: CFDI) {
   ...
   */
  _hasPaymentComplement(invoice: CFDI, paymentMap: Map<string, Date[]>): boolean {
  ...
  async updateSupplierDeductible(clientId: string, rfc: string, isDeductible: boolean): Promise<void> {
    try {
      ...
      const eligibleInvoices: CFDI[] = [];
      querySnapshot.forEach(doc => {
        const invoice = doc.data() as CFDI;
        if (!invoice.locked && 
  ...
  async evaluateDeductibility(clientId: string): Promise<{
    ...
    try {
      ...
      const calculateGravados = (invoice: CFDI) => {
        if (invoice.anual || invoice.usoCFDI?.startsWith('D')) {
          return { gravadoIVA: 0, gravadoISR: 0 };
        }
