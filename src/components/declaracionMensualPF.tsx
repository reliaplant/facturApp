            ...
            <table className="w-full text-xs relative">
              <thead className="sticky top-0 z-20">
                <tr className="after:absolute after:content-[''] after:h-[4px] after:left-0 after:right-0 after:bottom-0 after:shadow-sm">
                  ...
                  <th className="px-2 py-2.5 font-medium bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 text-center">Pago Imp.</th>
                  <th className="px-2 py-2.5 font-medium bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 text-center">Pago Serv.</th>
                    ...
                    >
                      ...
                      <td className="pl-6 pr-2 py-2.5 align-middle">
                        ...
                        <td className="px-2 py-2.5 align-middle text-center">
                          <Badge 
                          variant={declaracion.clientePagoImpuestos ? "secondary" : 
                          (!declaracion.clientePagoImpuestos && declaracion.fechaLimitePago && new Date() > new Date(declaracion.fechaLimitePago)) ? "destructive" : "default"}
                          className={`text-[10px] ${
                          declaracion.clientePagoImpuestos 
                          ? 'bg-green-100 text-green-800 hover:bg-green-200' 
                          : (!declaracion.clientePagoImpuestos && declaracion.fechaLimitePago && new Date() > new Date(declaracion.fechaLimitePago))
                          ? 'bg-red-100 text-red-800 hover:bg-red-200'
                          : 'bg-yellow-100 text-yellow-800 hover:bg-yellow-200'
                          }`}
                          >
                          {declaracion.clientePagoImpuestos ? 'Pagado' : 
                           (!declaracion.clientePagoImpuestos && declaracion.fechaLimitePago && new Date() > new Date(declaracion.fechaLimitePago)) 
                           ? `Vencida | Límite ${formatDate(declaracion.fechaLimitePago)}` 
                           : `Pendiente | Límite ${formatDate(declaracion.fechaLimitePago)}`}
                          </Badge>
                        </td>
