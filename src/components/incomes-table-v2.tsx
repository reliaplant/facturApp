...
export function IncomesTableV2({ year, invoices = [], disableExport = false, clientId, onInvoiceUpdate }: IncomesTableProps) {
  ...
  const uniqueReceptors = useMemo(() => {
    const receptorMap = new Map<string, string>();
    allFilteredInvoices.forEach(inv => {
      if (inv.rfcReceptor && !helpers.isPaymentComplement(inv)) {
        receptorMap.set(inv.rfcReceptor, inv.nombreReceptor || inv.rfcReceptor);
      }
    });
    return Array.from(receptorMap.entries())
  ...
  // Row renderer - keeps UI logic in component
  const renderInvoiceRow = (invoice: CFDI) => {
    ...
    return (
      ...
      >
        ...
        <td className="px-2 py-1 align-middle">
          <div className="flex flex-col">
            <span className="truncate max-w-[30ch] text-xs">{invoice.nombreReceptor}</span>
            <span className="text-purple-500 text-[10px]">
              {invoice.regimenFiscalReceptor || 'N/A'}
            </span>
          </div>
        </td>
        ...
        <td className="px-2 py-1 align-middle" onClick={(e) => e.stopPropagation()}>
          {isComplement ? null : highlightedEvaluated.includes(invoice.uuid || '') ? (
        ...
        <td 
          ...
          onDoubleClick={() => !isComplement && !invoice.locked && handleOpenDeductibility(invoice)}
        >
          {!isComplement && (
            highlightedEvaluated.includes(invoice.uuid || '') ? (
          ...
          <div className="flex items-center gap-2">
            ...
            <Select value={rfcFilter} onValueChange={setRfcFilter}>
              <SelectTrigger className="h-7 w-[320px] text-xs">
                <SelectValue placeholder="Todos los receptores" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">Todos los receptores</SelectItem>
                {uniqueReceptors.map(([rfc, nombre]) => (
            ...
            <table className="w-full text-xs relative">
              <thead className="sticky top-0 z-20">
                <tr className="after:absolute after:content-[''] after:h-[4px] after:left-0 after:right-0 after:bottom-0 after:shadow-[0_4px_8px_rgba(0,0,0,0.15)]">
                  ...
                  <th className="px-2 py-1.5 font-medium text-center bg-gray-100 dark:bg-gray-800 border-b-2 border-gray-300 dark:border-gray-600">¿Es ingreso?</th>
                  ...
                  <th className="pr-8 px-2 py-1.5 font-medium text-right bg-gray-100 dark:bg-gray-800 border-b-2 border-gray-300 dark:border-gray-600">IVA Retenido</th>
                </tr>
              </thead>
                    ...
                    <React.Fragment key={month}>
                      ...
                      <tr className="bg-gray-100 dark:bg-gray-800 border-t border-gray-300 dark:border-gray-600 hover:!bg-gray-100 dark:hover:!bg-gray-800">
                        ...
                        <td className="px-2 py-1.5 text-right text-xs text-gray-600 dark:text-gray-300">
                          ${monthlyTaxTotals[month].isr.toLocaleString('es-MX', { minimumFractionDigits: 2 })}
                        </td>
                        <td className="px-2 py-1.5 text-right text-xs text-gray-600 dark:text-gray-300">
                          ${monthlyTaxTotals[month].iva.toLocaleString('es-MX', { minimumFractionDigits: 2 })}
                        </td>
                        <td className="pr-7 px-2 py-1.5 text-right text-xs text-gray-600 dark:text-gray-300">
                          ${monthlyTaxTotals[month].ivaRetenido.toLocaleString('es-MX', { minimumFractionDigits: 2 })}
                        </td>
                      </tr>
                    </React.Fragment>
                  ))
